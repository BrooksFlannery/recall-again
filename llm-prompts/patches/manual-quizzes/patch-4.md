# [manual-quizzes] Patch 4: Add test stubs for manual quiz behavior
...
Patch 2 added migrations to create `quiz`/`quiz_item`, enable/force RLS, add policies, and grant access to `recall_app`.
...
## Test Stubs to Add
Add the following `.skip` tests to `src/server/trpc/routers/quiz.test.ts`:
- `quiz.createManual > returns quiz with N items`
- `quiz.createManual > only selects current user's facts (RLS)`
- `quiz.createManual > validates/bounds count`
- `quiz.getById > user cannot read another user's quiz (RLS)`
...
## Git Instructions
- Branch from: `main`
- Branch name: `manual-quizzes/patch-4-quiz-test-stubs`
- PR base: `main`
...
```bash
gh pr create --draft --title "[manual-quizzes] Patch 4: Add test stubs for manual quiz behavior" --body "Work in progress" --base main
```
